...
Always return exactly 3 meals. Use only the ingredients provided plus assumed pantry staples (oil, salt, pepper, water).`,
  ...
  function CoveTab() {
    ...
    // Always show empty shells ahead: next multiple of COVE_MIN_OPEN above shellCount
    // so after claiming or keeping, there are always empty slots to work toward
      ...
      <div>
        ...
        <div style={{textAlign:"center",fontSize:"0.72rem",color:T.textFaint,marginBottom:"1.25rem"}}>
          {ready&&!chestOpen
            ...
            : <span>{COVE_MIN_OPEN-shellCount>0?`${COVE_MIN_OPEN-shellCount} more shell${COVE_MIN_OPEN-shellCount===1?"":"s"} to open`:""}</span>
          }
        </div>
          ...
          <div style={{...card({background:"#fdf5e8",border:"1.5px solid "+sandHex}),marginBottom:"1rem"}}>
            {claimed ? (
              <div style={{textAlign:"center",padding:"0.5rem 0"}}>
                ...
                <div style={{fontSize:"0.8rem",color:"#633806",marginBottom:"0.85rem"}}>
                  {kid.shells>0
                    ? <>You still have <strong>{kid.shells}</strong> shell{kid.shells===1?"":"s"} — {kid.shells>=COVE_MIN_OPEN?"ready to open again!":"keep collecting to open again!"}</>
                    : "Keep earning shells to fill the beach again! 🐚"}
                </div>
