...
//
// Import coverage: the restore handler no longer has a per-key af_lighthouse guard.
// That guard (_lhOk / removeItem) was removed when App.jsx's backup-restore handler
// was rewritten for F-06/F-08/F-11 (allow-list import, not deny-list). lighthouse is
// now protected the same way every other SYNC_KEYS entry is: routed through the
// shared SYNC_KEYS allow-list, then sanitizeHouseholdData's own object-shape guard —
// proven directly against real sanitizeHouseholdData in "LH-1-B — sanitizeHouseholdData
// round-trip" above (LH-1-B3/B4/B5 drop bad shapes; B1/B2/B7 confirm valid blobs
// survive). These tests just confirm App.jsx's restore handler still wires lighthouse
// into that shared path, with no per-key special-casing.
describe("LH-8a-source — af_lighthouse survives backup restore via the generic SYNC_KEYS path", function() {
  var src;
  beforeAll(function() {
    var { readFileSync } = require("fs");
    var { join } = require("path");
  ...
  it("restore handler no longer special-cases af_lighthouse or af_safe_harbor by name", function() {
    expect(src).not.toMatch(/data\["af_lighthouse"\]/);
    expect(src).not.toMatch(/data\["af_safe_harbor"\]/);
  });

  it("restore handler gates SYNC_KEYS entries through the shared allow-list check", function() {
    expect(src).toMatch(/SYNC_KEYS\.indexOf\(bare\)\s*!==\s*-1/);
  });

  it("restore handler routes allow-listed entries through sanitizeHouseholdData before writing", function() {
    expect(src).toMatch(/sanitizeHouseholdData\(syncPayload\)/);
  });

  it("lighthouse is registered in SYNC_KEYS, so it reaches the shared allow-list + sanitize path", function() {
    expect(SYNC_KEYS.indexOf("lighthouse")).toBeGreaterThan(-1);
  });
});
